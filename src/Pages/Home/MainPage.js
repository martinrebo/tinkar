...
export default function MainPage() {
    ...
    const [props, set] = useSprings(dataCard.length, i => ({ ...to(i), from: from(i) })) // Create a bunch of springs using the helpers above
    ...
    const bind = useGesture(({ args: [index], down, delta: [xDelta], distance, direction: [xDir], velocity }) => {
        ...
        if (!down && gone.size === dataCard.length) setTimeout(() => gone.clear() || set(i => to(i)), 600)
    })
            ...
            <div style={{ color: theme.primary, background: theme.background }} >
                ...
                <div className="card-container ">
                    <div id="root-deck-card">
                        ...
                        {props.map(({ x, y, rot, scale }, i) => <animated.div key={i} style={{ transform: interpolate([x, y], (x, y) => `translate3d(${x}px,${y}px,0)`) }}>
